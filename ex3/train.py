...
def get_action(state, q_axis, q_table, epsilon=0.0):
    ...
    if choose_rand:
        # take random action
        return np.random.randint(0, q_table.shape[4])
    else:
        # take greedy action
        q_s = q_table[get_table_idx(state, q_axis)]
...
def update_q_value(old_state, action, new_state, gamma, reward, done, alpha, q_axis, q_table):
    ...
    if done:
        ...
        maxval = np.max(q_table[new_table_idx])

    q_table[old_table_idx][action] = q_table[old_table_idx][action] + alpha*(reward + gamma*maxval - q_table[old_table_idx][action])
