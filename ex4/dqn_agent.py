...
class DQNAgent(object):
    ...
    def update(self, buffer):
        ...
        # calculate the q(s,a)
        q_actions_policy = self.policy_net(batch.state)
        qs = torch.gather(input=q_actions_policy, dim=1, index=batch.action.type(torch.int64))

        # calculate q target 
        q_actions_target = self.target_net(batch.next_state)
        max_q = torch.max(input=q_actions_target, dim=1).values

        # set to zero where final state is reached
        max_q = torch.reshape(max_q, batch.not_done.size())*batch.not_done

        # calculate q target
        q_tar = batch.reward + self.gamma*max_q

        #calculate the loss 
        loss = (qs - q_tar.detach())**2 * 0.5
        loss = loss.sum()
    ...
    def get_action(self, state, epsilon=0.05):
        ...
        if choose_rand:
            ...
            # take greedy action
            q_actions_target = self.policy_net(torch.from_numpy(state).to(device))
            return torch.argmax(q_actions_target).item()
