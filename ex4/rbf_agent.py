...
class RBFAgent(object):
    ...
    def get_action(self, state, epsilon=0.0):
        ...
        # determine wether to choose greedy policy
        choose_rand = np.random.random() <= epsilon
        ...
        if choose_rand:
            ...
            # take greedy action
            q_values = [func.predict(f_state) for func in self.q_functions]
    ...
    def update(self, buffer):
        ...
        # variable for storing maximum q for each batch element
        max_q = np.zeros(self.batch_size)

        # extract next state batch elements in which the terminating state is not reached
        fns_not_done = f_next_state[batch.not_done.astype(bool), :]

        # predict q value for each possible action
        q_actions = np.array([func.predict(fns_not_done) for func in self.q_functions])

        # set maximum q for non terminating batch elements
        max_q[batch.not_done.astype(bool)] = np.max(q_actions, axis=0)

        # calculate q_target
        q_tar = batch.reward + self.gamma*max_q
