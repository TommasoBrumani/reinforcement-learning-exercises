    ...
    def plan(self, obs, t0, eval_mode=False):
        ...
        with Parallel(n_jobs=-1,) as parallel:  # we use joblib.Parallel to parallel the evaluation.
            ...
            for _ in range(self.iteration):
                ...
                partitioned_action_indices = np.argpartition(action_values, -self.num_topk)
                top_action_indices = partitioned_action_indices[-self.num_topk:]
                top_actions = actions[top_action_indices]

                # 4. compute new mean and std, note that we used momentum for mean
                _mean, _std = np.mean(top_actions, axis=0), np.std(top_actions, axis=0)
