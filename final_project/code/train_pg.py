...
def train(agent, env):
    ...
    while not done:
        ...
        # perform step and update observation
        obs, reward, done, _ = env.step(to_numpy(action))

        # update log prob and reward
        agent.record(act_logprob, reward)

        # increase sum and timestep
        reward_sum += reward
        timesteps += 1
...
# The main function
@hydra.main(config_path='cfg', config_name='pg_cfg')
def main(cfg):
    ...
    if cfg.model_path == 'default':
        print(f'{cfg.env_name}_params.pt')
